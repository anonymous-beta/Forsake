...
class ForsakeDB:
    ...
    def record_failed_login(self, username: str, ip_address: str = None):
        ...
        conn = self._get_conn()
        conn.execute(
            "INSERT INTO audit_log (user_id, action, details, ip_address, created_at) VALUES (NULL, ?, ?, ?, ?)",
            ("failed_login", username, ip_address, timestamp())
        )
        conn.commit()
        conn.close()
    ...
    def is_locked_out(self, username: str) -> bool:
        ...
        cutoff = (datetime.now() - timedelta(minutes=cfg.LOCKOUT_DURATION_MINUTES)).isoformat()
        conn = self._get_conn()
        count = conn.execute(
            """SELECT COUNT(*) FROM audit_log
               WHERE action = 'failed_login' AND details = ? AND created_at > ?""",
            (username, cutoff)
        ).fetchone()[0]
        conn.close()
        return count >= cfg.MAX_LOGIN_ATTEMPTS
