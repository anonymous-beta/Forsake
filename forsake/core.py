...
class Forsake:
    ...
    def deploy(self, domain: str, email: str = None,
        ...
        try:
            ...
            self.gophish.configure(self.admin_password, self.api_key)
        ...
        # Step 3: SSL
        try:
            self.ssl.obtain(self.domain, email)
            steps.append(("ssl", "ok"))
        except Exception as e:
            errors.append(f"SSL failed: {e}")
            try:
                self.ssl.generate_self_signed(self.domain)
        ...
        # Step 4: NGINX
        try:
            self.nginx.generate_config(self.domain)
            if self.nginx.install_config():
                steps.append(("nginx", "ok"))
            else:
                errors.append("NGINX config installed but failed validation/reload")
        except Exception as e:
            errors.append(f"NGINX config failed: {e}")

        # Step 5: Optional landing page
        if clone_url:
            try:
                path = self.cloner.clone(clone_url)
                self.cloner.inject_tracking(path)
...
{'='*60}
  ...
  ▶ Start:  sudo systemctl start forsake-gophish
  ⏹ Stop:   sudo systemctl stop forsake-gophish
  📊 Status: sudo systemctl status forsake-gophish nginx
...
"""
    ...
    def teardown(self, remove_data: bool = False):
        ...
        for svc in ["forsake-gophish", "forsake-web"]:
    ...
    def status(self) -> dict:
        ...
        gophish_running = run_command("systemctl is-active forsake-gophish")[0] == 0
        nginx_running = run_command("systemctl is-active nginx")[0] == 0
