...
# Rate-limit zones (idempotent if the same zone name is redefined)
...
server {{
    ...
    # Security headers (pure add_header — no third-party modules required)
    ...
    # Strip GoPhish identifying headers
    ...
    # Block common scanners / empty UA
...
# ── Admin interface (stricter limits) ─────────────────────────────────────
server {{
    listen 443 ssl http2;
    listen [::]:443 ssl http2;
    ...
    location / {{
        ...
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_ssl_verify off;   # local self-signed GoPhish admin cert
        proxy_buffering off;
    }}
}}
"""
