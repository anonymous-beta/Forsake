...
def hash_password(password: str) -> str:
    """Hash a password with bcrypt (preferred) or strong PBKDF2."""
    if _HAS_BCRYPT:
...
def verify_password(password: str, hashed: str) -> bool:
    """Verify a password against its stored hash (supports legacy SHA-256)."""
    try:
        ...
        # Legacy SHA-256 support for existing databases
        if ":" in hashed:
...
def run_command(cmd: Union[str, List[str]], timeout: int = 120, shell: bool = False) -> tuple:
    ...
    Prefer list form to avoid shell injection. shell=True is only for trusted internal use.
    """
